...
const Icons = {
    ...
    workspaces: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5">
            ...
            <path d="M6 21h12M12 18v3" strokeLinecap="round"/>
            <path d="M7 8h4M7 11h7" strokeLinecap="round"/>
        </svg>
    ),
    ...
    const columns = [
        {
            ...
            items: [
                ...
                { icon: Icons.workspaces, text: "Workspaces" }
            ]
        },
        {
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 pointer-events-none">
                {[...Array(15)].map((_, i) => (
                    <div key={i} className="absolute w-0.5 h-0.5 bg-[#d4af37] rounded-full opacity-20"
                        style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, animation: `fadeInUp ${3 + Math.random() * 5}s infinite alternate` }}
                    />
                ))}
            </div>
        </section>
